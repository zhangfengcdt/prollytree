...
use crate::advisor::{RiskTolerance, RecommendationType};
...
impl MarketResearchModule {
    ...
    pub async fn analyze_market(&self, symbol: &str, context: &AnalysisContext) -> Result<MarketAnalysisResult> {
        ...
        let similar_episodes = self.find_similar_market_conditions(&context.market_conditions).await?;
        ...
        let ai_insights = self.generate_ai_insights(symbol, &market_facts, &similar_episodes).await;
        
        // 5. Synthesize comprehensive analysis
        Ok(MarketAnalysisResult {
            fundamental_analysis: self.perform_fundamental_analysis(symbol, &market_facts).await?,
            technical_analysis: self.perform_technical_analysis(symbol).await?,
            sector_analysis: self.perform_sector_analysis(symbol).await?,
            sentiment_analysis: self.perform_sentiment_analysis(symbol, &similar_episodes).await?,
            ai_insights,
        })
    }

    async fn get_market_knowledge(&self, symbol: &str) -> Result<Vec<prollytree::agent::MemoryDocument>> {
        self.memory_system.semantic
            .get_entity_facts("market", symbol)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to retrieve market knowledge: {}", e))
    }

    async fn find_similar_market_conditions(&self, conditions: &MarketSnapshot) -> Result<Vec<prollytree::agent::MemoryDocument>> {
        // Search for episodes with similar market conditions
        let _search_tags = vec![
            "market_analysis",
            &conditions.market_trend.to_lowercase(),
        ];
    ...
    async fn get_analysis_procedures(&self) -> Result<Vec<prollytree::agent::MemoryDocument>> {
        self.memory_system.procedural
            .get_procedures_by_category("market_analysis")
            .await
            .map_err(|e| anyhow::anyhow!("Failed to get analysis procedures: {}", e))
    }

    async fn generate_ai_insights(&self, symbol: &str, market_facts: &[prollytree::agent::MemoryDocument], episodes: &[prollytree::agent::MemoryDocument]) -> String {
...
Provide 2-3 key insights in bullet format."#,
            ...
            match agent.prompt(&prompt).await {
                ...
                Err(_) => format!("Market analysis for {} shows mixed signals requiring careful evaluation", symbol),
            }
        } else {
            format!("Technical and fundamental analysis suggests {} requires further evaluation", symbol)
        }
    }

    async fn perform_fundamental_analysis(&self, symbol: &str, _market_facts: &[prollytree::agent::MemoryDocument]) -> Result<FundamentalAnalysis> {
    ...
    async fn perform_sentiment_analysis(&self, symbol: &str, _episodes: &[prollytree::agent::MemoryDocument]) -> Result<SentimentAnalysis> {
...
impl RiskAnalysisModule {
    ...
    pub async fn assess_risk(&self, context: &AnalysisContext, market_analysis: &MarketAnalysisResult) -> Result<RiskAssessmentResult> {
        ...
        let risk_breakdown = self.calculate_risk_breakdown(context, market_analysis).await?;
        
        // 4. Generate AI-powered risk insights if available
        let risk_factors = self.generate_risk_insights(context, market_analysis, &risk_episodes).await;
        
        // 5. Assess client alignment
        let client_risk_alignment = self.assess_client_alignment(context, &risk_breakdown).await?;
        ...
        Ok(RiskAssessmentResult {
            ...
            risk_breakdown,
            risk_factors,
            mitigation_recommendations: self.generate_mitigation_strategies(context, &risk_breakdown_clone).await,
            client_risk_alignment,
        })
    }

    async fn get_client_risk_history(&self, _client_id: &str) -> Result<Vec<prollytree::agent::MemoryDocument>> {
        self.memory_system.episodic
            .get_episodes_in_period(
    ...
    async fn calculate_risk_breakdown(&self, context: &AnalysisContext, market_analysis: &MarketAnalysisResult) -> Result<HashMap<RiskCategory, f64>> {
    ...
    async fn generate_risk_insights(&self, context: &AnalysisContext, market_analysis: &MarketAnalysisResult, _episodes: &[prollytree::agent::MemoryDocument]) -> Vec<String> {
        if let Some(ref client) = self.rig_client {
            let prompt = format!(
...
List 3-5 specific risk factors to monitor."#,
            ...
            let agent = client
                .agent("gpt-3.5-turbo")
                .preamble("You are a risk management expert. Focus on specific, actionable risk factors.")
                .max_tokens(250)
                .temperature(0.2)
                .build();
    ...
    fn generate_default_risk_factors(&self, context: &AnalysisContext) -> Vec<String> {
        vec![
            format!("Market volatility risk for {} sector exposure", context.symbol),
            "Interest rate sensitivity given current economic environment".to_string(),
            format!("Concentration risk based on {:?} risk tolerance", context.client_profile.risk_tolerance),
            "Liquidity risk during market stress periods".to_string(),
            "Sector-specific risks and regulatory changes".to_string(),
        ]
    }

    async fn assess_client_alignment(&self, context: &AnalysisContext, risk_breakdown: &HashMap<RiskCategory, f64>) -> Result<f64> {
        ...
        let alignment = match context.client_profile.risk_tolerance {
            RiskTolerance::Conservative => {
                if overall_risk > 0.7 { 0.5 } else if overall_risk > 0.5 { 0.7 } else { 0.9 }
            },
            RiskTolerance::Moderate => {
                if overall_risk > 0.8 { 0.6 } else if overall_risk < 0.3 { 0.7 } else { 0.85 }
            },
            RiskTolerance::Aggressive => {
                if overall_risk < 0.4 { 0.6 } else if overall_risk > 0.6 { 0.9 } else { 0.8 }
            },
        };
    ...
    async fn generate_mitigation_strategies(&self, context: &AnalysisContext, risk_breakdown: &HashMap<RiskCategory, f64>) -> Vec<String> {
        let mut strategies = Vec::new();
        
        for (risk_type, risk_level) in risk_breakdown {
            if *risk_level > 0.6 {
                let strategy = match risk_type {
                    RiskCategory::MarketRisk => "Consider position sizing and diversification across market sectors",
                    RiskCategory::LiquidityRisk => "Maintain adequate cash reserves and avoid illiquid positions",
                    RiskCategory::ConcentrationRisk => "Diversify holdings across different assets and sectors",
                    RiskCategory::InterestRateRisk => "Consider duration management and rate-sensitive asset allocation",
        ...
        match context.client_profile.risk_tolerance {
            RiskTolerance::Conservative => {
                strategies.push("Emphasize capital preservation and steady income generation".to_string());
            },
            RiskTolerance::Aggressive => {
                strategies.push("Consider using stop-loss orders and profit-taking strategies".to_string());
            },
            _ => {}
        }
...
impl ComplianceModule {
    ...
    pub async fn validate_compliance(&self, context: &AnalysisContext, risk_assessment: &RiskAssessmentResult) -> Result<ComplianceValidation> {
        ...
        let automated_actions = self.generate_compliance_actions(context, risk_assessment, &violations, &warnings).await;
    ...
    async fn get_client_restrictions(&self, client_id: &str) -> Result<Vec<prollytree::agent::MemoryDocument>> {
        self.memory_system.semantic
    ...
    async fn generate_compliance_actions(&self, _context: &AnalysisContext, risk_assessment: &RiskAssessmentResult, violations: &[ComplianceViolation], warnings: &[ComplianceWarning]) -> Vec<String> {
...
impl RecommendationModule {
    ...
    pub async fn generate_recommendation(
        ...
        let base_recommendation = self.determine_base_recommendation(
            market_analysis, risk_assessment, compliance_validation
        ).await?;
        ...
        let personalized_reasoning = self.generate_personalized_reasoning(
            base_recommendation, context, market_analysis, risk_assessment, &client_history
        ).await;
        ...
        Ok(PersonalizedRecommendation {
            ...
            follow_up_actions: self.generate_follow_up_actions(context, base_recommendation).await,
        })
    }
    ...
    async fn get_client_history(&self, _client_id: &str) -> Result<Vec<prollytree::agent::MemoryDocument>> {
        self.memory_system.episodic
            .get_episodes_in_period(
...
Keep it conversational and encouraging."#,
            ...
            match agent.prompt(&prompt).await {
                Ok(response) => response.trim().to_string(),
                Err(_) => self.generate_fallback_reasoning(recommendation, context, risk_assessment),
            }
        } else {
    ...
    fn generate_fallback_reasoning(&self, recommendation: RecommendationType, context: &AnalysisContext, risk_assessment: &RiskAssessmentResult) -> String {
    ...
    async fn extract_client_factors(&self, context: &AnalysisContext, risk_assessment: &RiskAssessmentResult) -> Vec<String> {
        vec![
            format!("Risk tolerance: {:?}", context.client_profile.risk_tolerance),
            format!("Investment timeline: {}", context.client_profile.time_horizon),
            format!("Primary goals: {}", context.client_profile.investment_goals.join(", ")),
            format!("Risk alignment score: {:.1}%", risk_assessment.client_risk_alignment * 100.0),
            format!("Portfolio value: ${:.0}", context.client_profile.portfolio_value),
            if !context.client_profile.restrictions.is_empty() {
                format!("Investment restrictions: {}", context.client_profile.restrictions.join(", "))
            } else {
    ...
    async fn generate_follow_up_actions(&self, _context: &AnalysisContext, recommendation: RecommendationType) -> Vec<String> {
        let mut actions = vec![
        ...
        if matches!(_context.client_profile.risk_tolerance, RiskTolerance::Conservative) {
