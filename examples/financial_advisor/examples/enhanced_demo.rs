...
use financial_advisor::advisor::{RiskTolerance, RecommendationType};
...
async fn main() -> Result<()> {
    println!("{}", "🏦 Enhanced Financial Advisor Demonstration".bright_blue().bold());
    println!("{}", "Showcasing advanced agent memory and multi-step workflows".dimmed());
    ...
    println!("{}", "📋 Demonstration 1: Multi-Client Personalized Analysis".bright_green().bold());
    ...
    println!("{}", "📋 Demonstration 2: Deep Research & Multi-Step Analysis".bright_green().bold());
    println!("{}", "━".repeat(50).dimmed());
    ...
    println!("{}", "📋 Demonstration 3: Portfolio Rebalancing & Risk Management".bright_green().bold());
    println!("{}", "━".repeat(50).dimmed());
    ...
    println!("{}", "📋 Demonstration 4: Learning from Outcomes".bright_green().bold());
    println!("{}", "━".repeat(50).dimmed());
    ...
    println!("{}", "📋 Demonstration 5: Advanced Memory Features".bright_green().bold());
    println!("{}", "━".repeat(50).dimmed());
    ...
    println!("📊 Total memories created: {}", stats.overall.total_memories);
    println!("💾 Storage utilization: {} MB", stats.overall.storage_size_mb);
    ...
    println!();
    println!("📂 Memory data persisted to: {}", storage_path.bright_cyan());
...
async fn await_demo_clients(advisor: &mut EnhancedFinancialAdvisor) -> Result<()> {
    ...
    println!("👤 {}", "Client 1: Sarah (Conservative Retiree)".bright_cyan());
    ...
    println!("👤 {}", "Client 2: Mike (Young Aggressive Investor)".bright_cyan());
    ...
    println!("👤 {}", "Client 3: Jennifer (Family Moderate Investor)".bright_cyan());
    
    advisor.set_current_client("jennifer_family").await?;
    advisor.update_client_risk_profile("jennifer_family", RiskTolerance::Moderate).await?;
...
async fn await_demo_deep_analysis(advisor: &mut EnhancedFinancialAdvisor) -> Result<()> {
    ...
    println!("   Growth Prospects: {}", market_analysis.fundamental_analysis.growth_prospects);
    println!("   Competitive Position: {}", market_analysis.fundamental_analysis.competitive_position);
    
    println!("📊 Technical Analysis:");
    println!("   Trend Direction: {}", market_analysis.technical_analysis.trend_direction);
    println!("   Support Levels: {:?}", market_analysis.technical_analysis.support_levels);
    
    println!("🏭 Sector Analysis:");
    println!("   Sector Trend: {}", market_analysis.sector_analysis.sector_trend);
    println!("   Relative Performance: {:.1}%", market_analysis.sector_analysis.relative_performance * 100.0);
    
    println!("💭 Sentiment Analysis:");
    println!("   Analyst Sentiment: {:.1}%", market_analysis.sentiment_analysis.analyst_sentiment * 100.0);
    println!("   Market Sentiment: {:.1}%", market_analysis.sentiment_analysis.market_sentiment * 100.0);
...
async fn await_demo_portfolio_management(advisor: &mut EnhancedFinancialAdvisor) -> Result<()> {
    ...
    let rebalancing_recommendations = advisor.analyze_portfolio_rebalancing("jennifer_family", holdings).await?;
    ...
    for (i, rec) in rebalancing_recommendations.iter().enumerate() {
        ...
        println!("      Reasoning: {}", 
                 rec.personalized_reasoning.split('.').next().unwrap_or("Analysis complete"));
    }
...
async fn await_demo_learning(advisor: &mut EnhancedFinancialAdvisor) -> Result<()> {
    ...
    println!("   Client Satisfaction: {:.1}/10", outcome.client_satisfaction.unwrap_or(0.0) * 10.0);
    println!("   Followed Advice: {}", if outcome.followed_recommendation { "Yes" } else { "No" });
    
    // Update the recommendation outcome
    advisor.update_recommendation_outcome(&recommendation.recommendation_id, outcome).await?;
...
async fn await_demo_memory_features(advisor: &mut EnhancedFinancialAdvisor) -> Result<()> {
    ...
    println!("   Operations/sec: {:.2}", stats.overall.operations_per_second);
    ...
    println!("   Expired memories cleaned: {}", optimization_report.expired_cleaned);
    println!("   Memories consolidated: {}", optimization_report.memories_consolidated);
    println!("   Memories archived: {}", optimization_report.memories_archived);
    println!("   Low-value memories pruned: {}", optimization_report.memories_pruned);
...
fn print_recommendation_summary(recommendation: &DetailedRecommendation, symbol: &str) {
    println!("   📊 {} Recommendation: {:?} ({:.1}% confidence)", 
             symbol.bright_yellow(),
             recommendation.base_recommendation,
             recommendation.confidence * 100.0);
    
    println!("   💭 Key Insight: {}", 
             recommendation.personalized_reasoning
                 .split('.')
                 .next()
                 .unwrap_or("Analysis complete")
                 .trim());
    
    println!("   ⚖️  Risk Score: {:.1}/10 (Alignment: {:.1}%)", 
    ...
    println!("   ⏱️  Processing: {:.2}s", 
             recommendation.execution_metadata.total_execution_time.num_milliseconds() as f64 / 1000.0);
