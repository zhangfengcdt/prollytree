...
impl<const D: usize> ProllyStorage<D> {
    ...
    fn make_storage_key(table_name: &str, key: &Key) -> Vec<u8> {
        match key {
            ...
            Key::None => format!("{}:__schema__", table_name).into_bytes(),
    ...
    fn parse_key_from_storage_key(storage_key: &[u8], table_prefix: &str) -> Key {
        ...
        let key_part = key_str.strip_prefix(&format!("{}:", table_prefix)).unwrap_or("");
...
impl<const D: usize> StoreMut for ProllyStorage<D> {
    async fn insert_schema(&mut self, schema: &Schema) -> Result<()> {
        ...
        self.store.insert(key, schema_data)
            .map_err(|e| Error::StorageMsg(format!("Failed to insert schema: {}", e)))?;
        
        // Cache the schema
        self.schemas.insert(schema.table_name.clone(), schema.clone());
    ...
    async fn append_data(&mut self, table_name: &str, rows: Vec<DataRow>) -> Result<()> {
        for row in rows {
            ...
            self.store.insert(storage_key, row_data)
...
mod tests {
    ...
    async fn test_basic_operations() {
        ...
        let key = Key::I64(1);
        storage.insert_data("users", vec![(key.clone(), row.clone())]).await.unwrap();
