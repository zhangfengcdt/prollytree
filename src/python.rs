...
use crate::{
    ...
    config::TreeConfig,
    git::{GitVersionedKvStore, types::StorageBackend},
    proof::Proof,
...
impl PyAgentMemorySystem {
    ...
    fn new(path: String, agent_id: String) -> PyResult<Self> {
        let memory_system = AgentMemorySystem::init(path, agent_id, None)
            .map_err(|e| PyValueError::new_err(format!("Failed to initialize memory system: {}", e)))?;
    ...
    fn store_conversation_turn(
        ...
        py.allow_threads(|| {
            ...
            runtime.block_on(async {
                memory_system.short_term
                    .store_conversation_turn(&thread_id, &role, &content, metadata_values)
                    .await
                    .map_err(|e| PyValueError::new_err(format!("Failed to store conversation: {}", e)))
            })
        })
    }
    ...
    fn get_entity_facts(
        ...
        py.allow_threads(|| {
            ...
            runtime.block_on(async {
                let facts = memory_system.semantic
                    .get_entity_facts(&entity_type, &entity_id)
                    .await
                    .map_err(|e| PyValueError::new_err(format!("Failed to get facts: {}", e)))?;
                
                Python::with_gil(|py| {
                    let results: PyResult<Vec<HashMap<String, Py<PyAny>>>> = facts.iter().map(|doc| {
                        let mut map = HashMap::new();
                        map.insert("id".to_string(), doc.id.clone().into_py(py));
                        map.insert("facts".to_string(), doc.content.to_string().into_py(py));
                        map.insert("confidence".to_string(), doc.metadata.confidence.into_py(py));
                        map.insert("source".to_string(), doc.metadata.source.clone().into_py(py));
                        Ok(map)
                    }).collect();
                    results
                })
    ...
    fn store_procedure(
        ...
        py.allow_threads(|| {
            ...
            let steps_values: Result<Vec<serde_json::Value>, _> = steps.iter()
            ...
            let conditions = prerequisites.map(|p| serde_json::Value::Array(
                p.into_iter().map(serde_json::Value::String).collect()
            ));
            
            runtime.block_on(async {
                memory_system.procedural
                    .store_procedure(&category, &name, &description, steps_values, conditions, priority)
                    .await
    ...
    fn get_procedures_by_category(
        ...
        py.allow_threads(|| {
            ...
            runtime.block_on(async {
                let procedures = memory_system.procedural
                    .get_procedures_by_category(&category)
                    .await
                    .map_err(|e| PyValueError::new_err(format!("Failed to get procedures: {}", e)))?;
                
                Python::with_gil(|py| {
                    let results: PyResult<Vec<HashMap<String, Py<PyAny>>>> = procedures.iter().map(|doc| {
                        let mut map = HashMap::new();
                        map.insert("id".to_string(), doc.id.clone().into_py(py));
                        map.insert("content".to_string(), doc.content.to_string().into_py(py));
                        map.insert("created_at".to_string(), doc.metadata.created_at.to_rfc3339().into_py(py));
    ...
    fn checkpoint(&self, py: Python, message: String) -> PyResult<String> {
        py.allow_threads(|| {
            ...
            runtime.block_on(async {
                memory_system.checkpoint(&message)
                    .await
                    .map_err(|e| PyValueError::new_err(format!("Failed to create checkpoint: {}", e)))
            })
        })
    }
    ...
    fn optimize(&self, py: Python) -> PyResult<HashMap<String, usize>> {
        py.allow_threads(|| {
            ...
            runtime.block_on(async {
                let report = memory_system.optimize()
                    .await
                ...
                result.insert("memories_consolidated".to_string(), report.memories_consolidated);
...
impl PyVersionedKvStore {
    ...
    fn insert(&self, key: &Bound<'_, PyBytes>, value: &Bound<'_, PyBytes>) -> PyResult<()> {
        ...
        store.insert(key_vec, value_vec)
    ...
    fn update(&self, key: &Bound<'_, PyBytes>, value: &Bound<'_, PyBytes>) -> PyResult<bool> {
        ...
        let mut store = self.inner.lock().unwrap();
        store.update(key_vec, value_vec)
    ...
    fn list_keys(&self, py: Python) -> PyResult<Vec<Py<PyBytes>>> {
        ...
        let py_keys: Vec<Py<PyBytes>> = keys.iter()
    ...
    fn status(&self, py: Python) -> PyResult<Vec<(Py<PyBytes>, String)>> {
        ...
        let py_status: Vec<(Py<PyBytes>, String)> = status.iter()
            .map(|(key, status_str)| {
                (PyBytes::new_bound(py, key).into(), status_str.clone())
            })
            .collect();
    ...
    fn create_branch(&self, name: String) -> PyResult<()> {
        ...
        store.create_branch(&name)
            .map_err(|e| PyValueError::new_err(format!("Failed to create and switch branch: {}", e)))?;
    ...
    fn log(&self) -> PyResult<Vec<HashMap<String, Py<PyAny>>>> {
        ...
        Python::with_gil(|py| {
            let results: PyResult<Vec<HashMap<String, Py<PyAny>>>> = history.iter().map(|commit| {
                let mut map = HashMap::new();
                map.insert("id".to_string(), commit.id.to_hex().to_string().into_py(py));
                map.insert("author".to_string(), commit.author.clone().into_py(py));
                map.insert("committer".to_string(), commit.committer.clone().into_py(py));
