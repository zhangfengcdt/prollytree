...
pub trait NodeStorage2<const N: usize>: Send + Sync {
    ...
    fn get_node_by_hash(&self, hash: &ValueDigest<N>) -> Node2<N>;
    ...
    fn insert_node(&mut self, hash: ValueDigest<N>, node: Node2<N>);
...
///
/// - `N`: The size of the value digest.
pub struct FileSystemNodeStorage2<const N: usize> {
    dir_path: PathBuf,
    _marker: PhantomData<ValueDigest<N>>,
}

impl<const N: usize> FileSystemNodeStorage2<N> {
...
impl<const N: usize> NodeStorage2<N> for FileSystemNodeStorage2<N> {
    fn get_node_by_hash(&self, hash: &ValueDigest<N>) -> Node2<N> {
...
///
/// - `N`: The size of the value digest.
pub struct HashMapNodeStorage2<const N: usize> {
...
impl<const N: usize> NodeStorage2<N> for HashMapNodeStorage2<N> {
    fn get_node_by_hash(&self, hash: &ValueDigest<N>) -> Node2<N> {
